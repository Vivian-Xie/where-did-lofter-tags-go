"""
Lofter Tag Monitor - scraper.py
每小时运行一次，爬取 tag 页的：
  1. 官方"参与"总数（HTML解析）
  2. 最新帖子列表（DWR API），记录每篇的 permalink + publishTime
每次爬取后立即结算当天数据，每天北京时间 0 点同时做昨日最终结算
"""

import json
import os
import re
import time
import requests
from datetime import datetime, timezone, timedelta
from pathlib import Path

# ── 配置 ──────────────────────────────────────────────────────────────────────
TAG_URL    = "https://www.lofter.com/tag/%E5%88%83%E6%81%92/new"
TAG_NAME   = "%E5%88%83%E6%81%92"
DWR_URL    = "http://www.lofter.com/dwr/call/plaincall/TagBean.search.dwr"
DATA_FILE  = "data/records.json"
LOGIN_KEY  = "LOFTER-PHONE-LOGIN-AUTH"
LOGIN_AUTH = os.environ.get("LOFTER_AUTH", "")
CST        = timezone(timedelta(hours=8))

HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
                  "AppleWebKit/537.36 (KHTML, like Gecko) "
                  "Chrome/120.0.0.0 Safari/537.36",
    "Host":     "www.lofter.com",
    "Referer":  TAG_URL,
}

# ── 工具函数 ──────────────────────────────────────────────────────────────────

def cst_now() -> datetime:
    return datetime.now(CST)

def load_data() -> dict:
    p = Path(DATA_FILE)
    if p.exists():
        raw = p.read_text(encoding="utf-8").strip()
        if raw:
            try:
                return json.loads(raw)
            except json.JSONDecodeError:
                print("[warn] records.json 解析失败，重置为空数据")
    return {"hourly": [], "daily": [], "today": {}}

def save_data(data: dict):
    Path("data").mkdir(exist_ok=True)
    with open(DATA_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

# ── 登录 session ──────────────────────────────────────────────────────────────

def get_session() -> requests.Session:
    session = requests.Session()
    session.headers.update(HEADERS)
    try:
        session.get("http://www.lofter.com/login",
                    params={"urschecked": "true"}, timeout=15)
    except Exception as e:
        print(f"[warn] 登录页请求失败: {e}")

    if LOGIN_AUTH:
        session.cookies.set(LOGIN_KEY, LOGIN_AUTH, domain="www.lofter.com")
    else:
        print("[warn] LOFTER_AUTH 环境变量为空")

    try:
        session.get("http://www.lofter.com/", timeout=15)
    except Exception as e:
        print(f"[warn] 主页请求失败: {e}")
    return session

# ── 爬取官方参与数 ────────────────────────────────────────────────────────────

def fetch_official_count(session: requests.Session):
    try:
        resp  = session.get(TAG_URL, timeout=15)
        html  = resp.text
        match = re.search(r'([\d.]+)(万?)参与', html)
        if not match:
            print("[warn] 未找到参与数")
            return None
        num = float(match.group(1))
        if match.group(2) == "万":
            num = int(num * 10000)
        else:
            num = int(num)
        print(f"[info] 官方参与数: {num}")
        return num
    except Exception as e:
        print(f"[error] fetch_official_count: {e}")
        return None

# ── 爬取最新帖子列表 ──────────────────────────────────────────────────────────

def fetch_new_posts(session: requests.Session, since_timestamp: int = 0) -> list:
    get_num   = 100
    got_num   = 0
    all_posts = []
    seen      = set()

    data = {
        "callCount":       "1",
        "httpSessionId":   "",
        "scriptSessionId": "${scriptSessionId}187",
        "c0-id":           "0",
        "batchId":         "870178",
        "c0-scriptName":   "TagBean",
        "c0-methodName":   "search",
        "c0-param0":       f"string:{TAG_NAME}",
        "c0-param1":       "number:0",
        "c0-param2":       "string:",
        "c0-param3":       "string:new",
        "c0-param4":       "boolean:false",
        "c0-param5":       "number:0",
        "c0-param6":       f"number:{get_num}",
        "c0-param7":       f"number:{got_num}",
        "c0-param8":       f"number:{int(time.time() * 1000)}",
    }

    while True:
        try:
            resp    = session.post(DWR_URL, data=data, timeout=20)
            content = resp.content.decode("utf-8")
        except Exception as e:
            print(f"[error] DWR 请求失败: {e}")
            break

        chunks = content.split("activityTags")[1:]
        if not chunks:
            print("[info] 无新帖子或已到最后一页")
            break

        stop = False
        for chunk in chunks:
            pm = re.search(r's\d+\.permalink="(.*?)"', chunk)
            if not pm:
                continue
            permalink = pm.group(1)
            if permalink in seen:
                continue
            seen.add(permalink)

            ts_m = re.search(r's\d+\.publishTime=(\d+);', chunk)
            if not ts_m:
                continue
            pub_ts = int(ts_m.group(1))

            if since_timestamp and pub_ts <= since_timestamp:
                stop = True
                break

            pub_str = datetime.fromtimestamp(
                pub_ts / 1000, tz=CST).strftime("%Y-%m-%d %H:%M:%S")
            all_posts.append({
                "permalink":      permalink,
                "publishTime":    pub_ts,
                "publishTimeStr": pub_str,
            })

        if stop:
            print("[info] 已追溯到上次记录时间，停止翻页")
            break

        got_num += get_num
        if len(chunks) < get_num:
            break

        last_ts_m = re.search(r's\d+\.publishTime=(\d+);', chunks[-1])
        if not last_ts_m:
            break
        data["c0-param6"] = f"number:{get_num}"
        data["c0-param7"] = f"number:{got_num}"
        data["c0-param8"] = f"number:{last_ts_m.group(1)}"
        time.sleep(0.5)

    print(f"[info] 本次获取新帖: {len(all_posts)} 篇")
    return all_posts

# ── 结算当天数据（每次爬取都调用）────────────────────────────────────────────

def settle_today(data: dict, today_str: str):
    today_hourly = [r for r in data["hourly"] if r.get("date") == today_str]
    total_new    = sum(r.get("new_posts_count", 0) for r in today_hourly)
    counts       = [r["official_count"] for r in today_hourly
                    if r.get("official_count") is not None]
    start_count  = counts[0]  if counts else None
    latest_count = counts[-1] if counts else None
    official_growth = (latest_count - start_count) if (
        start_count is not None and latest_count is not None) else None
    discrepancy = (official_growth - total_new) if official_growth is not None else None

    data["today"] = {
        "date":            today_str,
        "total_new_posts": total_new,
        "official_start":  start_count,
        "official_latest": latest_count,
        "official_growth": official_growth,
        "discrepancy":     discrepancy,
        "hours_recorded":  len(today_hourly),
    }
    print(f"[info] 今日实时结算: 新帖={total_new}, 官方增长={official_growth}, 差异={discrepancy}")

def settle_yesterday(data: dict, yesterday_str: str):
    if any(d["date"] == yesterday_str for d in data["daily"]):
        print(f"[info] {yesterday_str} 已结算，跳过")
        return
    yest_hourly = [r for r in data["hourly"] if r.get("date") == yesterday_str]
    if not yest_hourly:
        return
    total_new   = sum(r.get("new_posts_count", 0) for r in yest_hourly)
    counts      = [r["official_count"] for r in yest_hourly
                   if r.get("official_count") is not None]
    start_count = counts[0]  if counts else None
    end_count   = counts[-1] if counts else None
    official_growth = (end_count - start_count) if (
        start_count is not None and end_count is not None) else None
    discrepancy = (official_growth - total_new) if official_growth is not None else None
    data["daily"].append({
        "date":            yesterday_str,
        "total_new_posts": total_new,
        "official_start":  start_count,
        "official_end":    end_count,
        "official_growth": official_growth,
        "discrepancy":     discrepancy,
    })
    print(f"[info] 昨日最终结算 {yesterday_str}: 新帖={total_new}, 差异={discrepancy}")

# ── 主函数 ────────────────────────────────────────────────────────────────────

def run():
    now           = cst_now()
    now_str       = now.strftime("%Y-%m-%d %H:%M")
    today_str     = now.strftime("%Y-%m-%d")
    yesterday_str = (now - timedelta(days=1)).strftime("%Y-%m-%d")

    print(f"\n====== {now_str} CST 开始爬取 ======")

    data = load_data()

    since_ts = 0
    if data["hourly"]:
        since_ts = data["hourly"][-1].get("earliest_post_ts", 0)

    session        = get_session()
    official_count = fetch_official_count(session)
    new_posts      = fetch_new_posts(session, since_ts)

    new_count   = len(new_posts)
    earliest_ts = min((p["publishTime"] for p in new_posts), default=since_ts)

    data["hourly"].append({
        "time":             now_str,
        "date":             today_str,
        "official_count":   official_count,
        "new_posts_count":  new_count,
        "earliest_post_ts": earliest_ts,
    })

    cutoff = (now - timedelta(days=30)).strftime("%Y-%m-%d")
    data["hourly"] = [r for r in data["hourly"] if r.get("date", "") >= cutoff]

    # 每次都结算今天
    settle_today(data, today_str)

    # 0 点时额外结算昨天
    if now.hour == 0:
        settle_yesterday(data, yesterday_str)

    save_data(data)
    print(f"[info] 数据已保存 → {DATA_FILE}")
    print(f"====== 爬取完成 ======\n")


if __name__ == "__main__":
    run()
